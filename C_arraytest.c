...
static PyObject *intfcn1(PyObject *self, PyObject *args)
{
	...
	long int **cin, **cout;              // The C integer 2D arrays to be created to point to the 
...
/* ==== Create integer 2D Carray from PyArray ======================
    Assumes PyArray is contiguous in memory.
    Memory is allocated!                                    */
long int **pyint2Darray_to_Carrayptrs(PyArrayObject *arrayin)  {
	long int **c, *a;
	int i,n,m;
...
//	a=(int *) arrayin->data;  /* pointer to arrayin data as int */
	a=(long int *) PyArray_DATA(arrayin);  /* pointer to arrayin data as int */
	for ( i=0; i<n; i++)  {
		c[i]=a+i*m;  }
	return c;
}
...
void free_Cint2Darrayptrs(long int **v)  {
	free((char*) v);
}
...
//	if (mat->descr->type_num != NPY_LONG || mat->nd != 2)  {
	if (PyArray_TYPE(mat) != NPY_LONG || PyArray_NDIM(mat) != 2)  {
		PyErr_SetString(PyExc_ValueError,
